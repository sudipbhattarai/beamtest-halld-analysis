...
int delta_y_steps = 400; // the number of steps between the vertical limit switches
float y_mm_per_step = 0.005;
float x_mm_per_step = 0.005;
...
float get_home() {
    return delta_y_steps / 2; // this can be used to find the home position. defaulted to set in the middle.
}
...
void send_command(int fd, char *command)
{
    ...
    printf("COMMAND SENT: %s",full_command); // debug text
...
void remove_first_element(char arr[], int *size) {
    if (*size <= 0) return;  // Don't proceed if the array is empty
    ...
    (*size)--;  // Decrease the size of the array (logical size)
}
...
static void lower_limit_button_clicked_cb(GtkWidget *button, gpointer data) {
    if (show_confirmation_dialog(button)) {
        send_command(fd, "C, I2M0, R");
    }
}
...
static void move_left_button_clicked_cb(GtkWidget *button, gpointer data)
{
    if (show_confirmation_dialog(button))
    {
        ...
        int steps = (int)((distance / x_mm_per_step));
        char command[128];
...
static void move_down_button_clicked_cb(GtkWidget *button, gpointer data)
{
    if (show_confirmation_dialog(button))
    {
        ...
        double distance = strtod(distance_str, NULL); // Convert the string to a double
        // Convert the distance to steps by dividing by 0.0025 and rounding to the nearest integer
        int steps = (int)((distance / y_mm_per_step));
        char command[128];
...
static void status_button_clicked_cb(GtkWidget *button, gpointer data)
{
	...
	printf("before the read is attempted\n");
    int n = read(fd, buffer, sizeof(buffer) - 1); // Leave space for null terminator
	printf("after the read is attempted\n");
    if (n < 0)
    {
        perror("Read failed");
    }
...
static void get_position_button_clicked_cb(GtkWidget *button, gpointer data) // updated function
{
    char buffer[32];
    int n;
    int position;
    ...
    for (int i = 0; i < 2; ++i) // Loop over 2 elements
    {
		...
		write(fd, commandCopy, strlen(commandCopy)); // needs to be written without any termination characters for some reason...
		printf("I'm sending the raw command %s with no termination characters\n", commandCopy);
		
		sleep(1); // wait to read so that the VXM can respond

        // Read response
        n = read(fd, buffer, sizeof(buffer) - 1);
		
		while (buffer[0] != '+' && buffer[0] != '-') // clean up the buffer (will probably have junk characters at the start)
		{
			remove_first_element(buffer, &n);
        ...
        else
        {
			...
			if (i == 0)
            	distance_mm = 1.0 * position * x_mm_per_step; // Convert position to mm
			if (i == 1)
		distance_mm = -1.0 * position * x_mm_per_step;
...
static void camera_button_clicked_cb(GtkWidget *button, gpointer data) {
    // Capture an image using raspistill
    system("raspistill -o test.jpg");

    // Display the image using ImageMagick
...
{
    send_command(fd, "C, setM1M5, setM2M4");
	send_command(fd, "SA1M800, SA2M400, R"); // set motors 1 and 2 to max power and 1200/200 steps/sec
}
...
{	
    ...
    GtkWidget *start_limit_button = gtk_button_new_with_label("Lower Limit Switch (Y)");
    g_signal_connect(start_limit_button, "clicked", G_CALLBACK(lower_limit_button_clicked_cb), NULL);
    gtk_widget_set_size_request(start_limit_button, width, height); // Width = width, Height = height

    GtkWidget *end_limit_button = gtk_button_new_with_label("Out Limit Switch (X)");
    ...
    GtkWidget *move_button = gtk_button_new_with_label("Move Down (Y)");
    g_signal_connect(move_button, "clicked", G_CALLBACK(move_down_button_clicked_cb), NULL);
    gtk_widget_set_size_request(move_button, width, height); // Width = width, Height = height

    GtkWidget *move_closer_button = gtk_button_new_with_label("Move Up (Y)");
    g_signal_connect(move_closer_button, "clicked", G_CALLBACK(move_up_button_clicked_cb), NULL);
    ...
    GtkWidget *status_button = gtk_button_new_with_label("View Camera");
    g_signal_connect(status_button, "clicked", G_CALLBACK(camera_button_clicked_cb), NULL);
    ...
    GtkWidget *move_left_button = gtk_button_new_with_label("Move Out (X)");
    ...
    GtkWidget *move_right_button = gtk_button_new_with_label("Move In (X)");
    ...
    gtk_grid_attach(GTK_GRID(movement_commands_grid), start_limit_button, 1, 0, 1, 1);
    gtk_grid_attach(GTK_GRID(movement_commands_grid), end_limit_button, 0, 0, 1, 1);
